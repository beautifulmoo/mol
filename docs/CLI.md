...
| 항목 | 설명 |
|------|------|
| **종료 코드** | 성공 **`0`**, 실패 **`1`**. `maintenance`·`discoverycli`·`applycli` 패키지는 **`os.Exit`를 호출하지 않고** 상위 `main`이 `os.Exit` 한다. |
| **도움말 언어** | `-h` / `--help` 본문 및 **`--apply-update`** 관련 진단 메시지는 **영문**으로 출력한다(로캘 미설치 OS 대비). `--discovery` 도움말도 영문. |

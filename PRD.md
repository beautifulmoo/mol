...
- **소스 레이아웃**: Discovery·Discovery CLI(`discoverycli`)·**업데이트 적용 CLI(`applycli`)**·**설치 버전 CLI(`versionscli`: `--versions-list` / `--versions-switch`)**·**호스트 정보 CLI(`hostinfocli`: `--host-info`)**·로컬 `hostinfo` 패키지·HTTP API·서비스 상태·웹 정적 파일은 **`maintenance/`** 아래 패키지로 구성된다. 루트 `main.go`는 `contrabass-agent/maintenance` 만 import한다. **설정(YAML)** 은 **`internal/config`** 에서 로드한다. **업데이트/롤백 셸**은 루트 `update.sh`·`rollback.sh` 를 소스로 하여 **`internal/updatescripts/`** 에 복사 후 **`//go:embed`** 로 바이너리에 포함한다(`Makefile`이 빌드 전 동기화). Discovery용 IPv4 brd 자동 수집 규칙과 동일한 의도의 **참고 셸** `brd_for_bm.sh` 를 저장소 루트에 둔다(§3.1.1).
- **진입점·종료 코드**: 루트 `main.go`는 빌드 시 주입되는 **`main.VersionKey`**(ldflags `-X main.VersionKey=…`, `Makefile` 기본값은 **`./scripts/build-version.sh`** 가 출력하는 **`git describe --tags --long --always` 전체 문자열**, 예: `0.4.4-4-gc44d420`; 필요 시 **`make build VERSION_KEY=…`** 로 덮어쓸 수 있음)과 **`main()`** 만 두고, **`contrabass-moleU -cfg <파일>`**(비어 있지 않은 경로)인 **서비스 모드**에서만 Gin 리버스 프록시(`Server.HTTPPort`)를 `go`로 기동한 뒤 **`maintenance.Run(main.VersionKey, os.Args)`** 를 호출하고, 그 반환값으로 **`os.Exit`** 한다. `--nic-brd`·`--discovery`·`--apply-update`·`--versions-list`·`--versions-switch`·`--host-info`·`-h` 등 **CLI 전용** 실행 시에는 Gin을 띄우지 않는다. **`maintenance.Run(buildVersionKey, args []string) int`** 는 **명령줄은 `args` 인자로만** 받으며, 성공·오류는 **`0` 또는 `1`** 반환만으로 알린다(`maintenance` 패키지에서 `os.Exit`를 호출하지 않음). HTTP·Discovery 서비스 기동·`-h`·`--version`·`--nic-brd`·`--apply-update`·`--versions-list`·`--versions-switch`·`--host-info`·`-cfg` 등의 분기와 **`//go:embed web/*`**(웹 정적 파일)은 **`maintenance/maintenance.go`** 에 모은다. **`discoverycli.Run`** 은 **`contrabass-moleU --discovery`**, **`applycli.Run`** 은 **`--apply-update`**, **`versionscli.RunList` / `RunSwitch`** 는 **`--versions-list` / `--versions-switch`**, **`hostinfocli.Run`** 은 **`--host-info`** 경로에서 각각 **종료 코드 `int`** 를 반환한다(`os.Exit` 없이).
...
- **`--host-info`**: **`-cfg <설정 파일>`** 과 **`<self|원격 IP>`** 한 인자. **로컬 유지보수 HTTP**가 떠 있어야 한다. 항상 **이 머신의** maintenance로 `GET …/host-info`를 보낸다 — **`self`**면 `ip` 없이(또는 `?ip=self`) **HTTP `/self`와 동일**한 로컬 호스트 정보; **원격 IP**면 서버가 해당 주소로 **UDP 유니캐스트 Discovery**(`DoDiscoveryUnicast`)만 수행한다(원격 에이전트 Gin으로 HTTP 프록시하지 않음). 표준 출력은 DISCOVERY_RESPONSE 주요 필드를 영문 라벨로 표 형태로 출력한다. **`-h` 도움말 순서**: `-h` 다음에 `-version` 다음 **`--host-info`** 가 오고 그 다음 **`--nic-brd`**(그 외 옵션은 기존과 동일).
...
- **`--apply-update`**: **`-cfg <설정 파일>`** 과 **`<self|원격 IP>`**, **`<bundle.tar.gz>`** 두 인자가 필요하다. **로컬 유지보수 HTTP**(`MaintenanceListenAddress`·`MaintenancePort`; `0.0.0.0`/빈 값은 클라이언트에서 `127.0.0.1` 등으로 접속)가 떠 있어야 한다. (1) 번들을 임시 디렉터리에 풀어 **서버와 동일한 검증**(manifest·해시·ELF·`--version`, §5.5.3) 후 **번들 버전 키**를 얻는다. (2) **현재 버전**은 `GET …/self`로 조회 — **self**는 유지보수 API, **원격 IP**는 `http://<ip>:Server.HTTPPort` + `APIPrefix` + `/self` (적용 전 **TCP로 해당 주소:포트 연결** 가능 여부 확인). (3) **`internal/config.StagingUpdateAvailable`(번들 키, 현재 키)** 가 참일 때만 이후 단계로 진행한다. (4) **self**: `POST …/upload`(multipart `bundle`) 후 `POST …/apply-update` JSON `{"version":"<키>"}`. (5) **원격**: 로컬 유지보수에 **`POST …/apply-update` multipart**(`ip`, `bundle`) — 서버가 **원격 `POST …/upload`에 직접 전송** 후 원격 apply-update(self)를 호출한다(로컬 스테이징 없이 원격만 갱신, §5.5.3 multipart 원격 적용과 동일). **CLI 도움말·진단 메시지**는 로캘 미설치 환경을 고려해 **영문**으로 출력한다. 설정 YAML 검증 실패·번들 검증 오류 등 CLI 경로에서 노출되는 메시지도 동일 정책을 따른다(HTTP API 본문으로 돌아오는 서버 메시지는 구현에 따라 한글일 수 있음).
- **`--versions-list`**: **`-cfg <설정 파일>`** 과 **`<self|원격 IP>`**. 로컬 유지보수 HTTP가 떠 있어야 한다. `GET …/versions/list` — **self**는 쿼리 없음, **원격 IP**는 `?ip=` 프록시. 설치된 버전·current/previous 플래그를 표로 출력(영문 헤더).
- **`--versions-switch`**: **`-cfg <설정 파일>`** 과 **`<self|원격 IP>`**, **`<버전 키>`**. `POST …/versions/switch-current` — 로컬은 JSON `version`+`ip:self`, 원격은 적용 전 **`TCP`로 `<ip>:Server.HTTPPort`** 연결 확인 후 동일 API 프록시. 내장 `update.sh`를 `systemd-run`으로 실행하는 경로는 웹 UI와 동일.
...
- [ ] **CLI**: **`-cfg <파일>`** 로만 HTTP 서버 + Discovery 기동; 인자 없이 실행 시 안내 후 종료; `-h`/`--help`(도움말 본문은 영문; 옵션 순서: `-h`, `-version`, **`--host-info`**, `--nic-brd`, …); `--version`/`-version`; **`--host-info -cfg <file> <self|ip>`**(GET host-info, 원격은 유니캐스트 Discovery); `--nic-brd`; **`--discovery`**(UDP만, `--dest-port`/`--src-port`/`--timeout`, 결과에 **`version=`**); **`--apply-update -cfg <file> <self|ip> <bundle.tar.gz>`**(번들 사전 검증·`StagingUpdateAvailable`·self는 upload+apply·원격은 multipart로 서버 경유, 메시지 영문); **`--versions-list -cfg <file> <self|ip>`** / **`--versions-switch -cfg <file> <self|ip> <version-key>`**(REST `versions/list`, `versions/switch-current` 대응, 메시지 영문)
